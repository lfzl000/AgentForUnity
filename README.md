...
Right-click selected objects in **Hierarchy** or **Project**, or a component's Inspector header, and choose **添加到 AgentForUnity**. The Agent window opens with removable context chips; multiple objects and successive additions are retained. Object snapshots include asset paths/GUIDs or scene and hierarchy paths, object IDs, and component types. They identify the object for the Agent without embedding an entire asset's contents.

Console logs also support **添加到 AgentForUnity**, including their message and stack trace. In both Unity 2022.3 and Unity 6, first left-click/select the logs, then right-click inside the Console content area to attach that selection. The menu hooks the Console's actual IMGUI host, including when the window is docked; the toolbar and dock tabs keep Unity's normal behavior. Context is captured when the menu opens, so later log arrivals do not change the attachment. The existing **+ Console** picker remains available. Attached context is sent with both **Send** and **Steer**, including messages containing only attachments; adding context never submits a message automatically.
